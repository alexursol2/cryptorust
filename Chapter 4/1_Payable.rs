...
pub trait ZombieHelper: storage::Storage {
    ...
    #[payable("EGLD")] // a payable endpoint accepting EGLD
    #[endpoint]
    fn level_up(&self, zombie_id: usize){
        ...
        // The function should require that self.call_value().egld_value() is equal to level_up_fee
        self.zombies(&zombie_id).update(|my_zombie| my_zombie.level += 1); // increment this zombie's level
    }
}
